...
from portaldata.models import Indicator, IndicatorData
...
class SummaryResponse:
    ...
    #             question=question, value=clean_label).count()
    #         data.append(count)
    ...
    #     for label in labels:
    #         count = Answer.objects.filter(
    #             question=question, value=label).count()
    #         data.append(count)

    #     values_rating = Answer.objects.filter(
    #         question=question).values_list('value', flat=True)
    ...
    def _process_multiselect_type(self, indicator: Indicator) -> str:
        bar_chart = ChartBar(
            chart_id=f"barchart_{indicator.pk}", chart_name=indicator.label)
    ...
    def generate(self):
        html_str = []
        html_str.append(self._process_multiselect_type(self.indicator))
        print(html_str)
