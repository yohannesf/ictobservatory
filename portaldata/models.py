...
DATA_TYPE = namedtuple(
    'DATA_TYPE', 'currency number decimal percentage text select text_area url email date'
)._make(range(10))

IND_ASSIGNED_TO = namedtuple(
    'IND_ASSIGNED_TO', 'MEMBER_STATES ORGANIZATIONS SADC')._make(['M', 'O', 'S'])


INDICATORDATA_STATUS = namedtuple(
    'STATUS', 'draft ready returned validated'
)._make(range(1, 5))
...
class FocusArea(models.Model):
    ...
    sn = models.IntegerField(
        verbose_name="S/N", unique=True, blank=True)
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Focus_Area_Created_By", on_delete=models.DO_NOTHING)
    last_update = models.DateTimeField(auto_now=True)
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Focus_Area_Updated_By", on_delete=models.DO_NOTHING)
    ...
    def count_core_indicators(self):
        ...
        ind_count = Indicator.objects.filter(
            focus_area=self).filter(indicator_type='Core').count()
        return ind_count

    def count_extended_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self).filter(indicator_type='Extended').count()
        return ind_count

    def count_active_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self).filter(status='Active').count()
        return ind_count

    def count_archived_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self).filter(status='Archived').count()
        return ind_count

    def count_required_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self).filter(required=True).count()
        return ind_count

    def count_active_required_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self, required=True).filter(status='Active').count()
        return ind_count

    def count_optional_indicators(self):

        ind_count = Indicator.objects.filter(
            focus_area=self).filter(required=False).count()
        return ind_count
    ...
    def count_active_required_indicators_by_assignment(self, assignedto):
        ...
        ind_count = Indicator.objects.filter(focus_area__focusarea_status=True,
                                             focus_area=self, required=True, indicator_assigned_to=assignedto).filter(status='Active').count()
        return ind_count

    def count_completed_indicators_by_assignment(self, assignedto, memberstate, reporting_year):

        ind_count = IndicatorData.objects.filter(indicator__focus_area=self, indicator__focus_area__focusarea_status=True,
    ...
    def count_indicators(self):
        ind_count = Indicator.objects.filter(focus_area=self).exclude(
            indicator_assigned_to=IND_ASSIGNED_TO.ORGANIZATIONS).exclude(
...
class MemberState(models.Model):
    ...
    memberstate_status = models.BooleanField(
        choices=MEMBERSTATE_STATUS_CHOICES,
        default=True,
    )
...
class Indicator(models.Model):
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Indicator_Created_By", on_delete=models.DO_NOTHING)

    last_update = models.DateTimeField(auto_now=True)

    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Indicator_Updated_By", on_delete=models.DO_NOTHING)
    ...
    def count_completed_required_indicators(self):
        ...
        ind_count = IndicatorData.objects.filter(indicator=self,
    ...
    def get_revision_request(self, org, reporting_year):
        ind_count = IndicatorData.objects.filter(reporting_year=reporting_year,
                                                 indicator__focus_area__focusarea_status=True,
...
class AssignedIndicator(models.Model):
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="IndicatorAssignment_Created_By", on_delete=models.DO_NOTHING)

    last_update = models.DateTimeField(auto_now=True)

    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="IndicatorAssignment_Updated_By", on_delete=models.DO_NOTHING)
    ...
    class Meta:
        unique_together = (
            'indicator', 'assigned_to_organisation')
...
class ReportingPeriod(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.current:
            print(self.current)
            ReportingPeriod.objects.update(
                current=Q(pk=self.pk)
            )
...
class IndicatorData(models.Model):
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Data_Created_By", on_delete=models.DO_NOTHING)

    last_update = models.DateTimeField(auto_now=True)

    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="Data_Last_Updated_By", on_delete=models.DO_NOTHING)
    ...
    class Meta:
        unique_together = (
            'reporting_year', 'indicator', 'member_state')
...
def update_usd(sender, instance, **kwargs):
    ...
    post_save.disconnect(update_usd, sender=IndicatorData,
                         dispatch_uid="update_currency_usd")
    instance.save()
    post_save.connect(update_usd, sender=IndicatorData,
                      dispatch_uid="update_currency_usd")
...
class IndicatorDataValidationHistory(models.Model):
    ...
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="revision_last_updated_by", on_delete=models.DO_NOTHING)
...
class GeneralIndicator(models.Model):
    ...
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="GeneralIndicator_Last_Updated_By", on_delete=models.DO_NOTHING)
...
class GeneralIndicatorData(models.Model):
    ...
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="GeneralIndicatorData_Last_Updated_By", on_delete=models.DO_NOTHING)
    ...
    class Meta:
        ...
        UniqueConstraint(fields=['general_indicator',
                         'reporting_year'], name='double_entry')

        unique_together = (
            'general_indicator', 'reporting_year')
...
class Published(models.Model):
    ...
    reporting_year = models.CharField(unique=True,
    ...
    updated_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="PublishedYear_Last_Updated_By", on_delete=models.DO_NOTHING)
...
class Chart(models.Model):
    ...
    indicators_list = models.ManyToManyField(
        Indicator, through='ChartConfig')
...
class ChartConfig(models.Model):
    ...
    extra_calculation = models.CharField(
        max_length=50,
        choices=EXTRA_CALCULATION_CHOICES,
        blank=True
